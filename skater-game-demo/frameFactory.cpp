...
FrameFactory::~FrameFactory()
{
  ...
  //free multiTexture containers
  for (auto& mti : multiTextures) 
  {
    for (auto& mtii : mti.second)
      SDL_DestroyTexture(mtii);
  }

  //free multiFrame containers
  for (auto& mfi : multiFrames) 
  {
    for (auto& mfii : mfi.second)
      delete mfii;
  }
}
...
std::vector<Frame*> FrameFactory::getFrames(const std::string& name) {
  ...
  //make rough estimate of frame size
  int width = surface->w/numberOfFrames;
  int height = surface->h;

  //if specific frame size exists, use it
  ...
  //create sprite sheet w/ these values
